<template>
  <div class="payment-method-management px-6 py-6 bg-slate-50 min-h-full">
    <!-- 搜索区域 -->
    ...
    <a-card class="ui-card mb-4" :bordered="false">
      <a-space>
        <a-button type="primary" @click="openAddModal" v-permission="['system:payment:create']" class="ui-btn-cta">
          <template #icon><plus-outlined /></template>
          新增支付方式
        </a-button>
        <a-button @click="loadPaymentMethodList" class="ui-btn-secondary">
          <template #icon><reload-outlined /></template>
          刷新
        </a-button>
      </a-space>
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'action'">
            <a-space>
              <a-button type="link" size="small" @click="openEditModal(record as PaymentMethod)" v-permission="['system:payment:update']" class="text-blue-600 font-medium">编辑</a-button>
              <a-popconfirm
                title="确定要切换状态吗？"
                @confirm="toggleStatus(record as PaymentMethod)"
              >
              ...
              >
                <a-button type="link" size="small" danger v-permission="['system:payment:delete']" class="font-medium">删除</a-button>
              </a-popconfirm>
            </a-space>
          </template>
        </template>
    ...
    >
      ...
      >
        ...
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="代码 (可选)" name="code">
              <a-input v-model:value="formData.code" placeholder="如 T/T" :maxlength="50" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
...
const loadPaymentMethodList = async () => {
  try {
    ...
    if (response.data?.code === 200) {
      paymentMethodList.value = response.data.data.records || []
...
// 打开编辑弹窗
const openEditModal = (record: PaymentMethod) => {
...
const handleSave = async () => {
  try {
    ...
    if (editingId.value) {
      ...
      response = await updatePaymentMethod(id, formData as any)
    } else {
      response = await addPaymentMethod(formData as any)
    }
...
const handleDelete = async (id: number | string) => {
  try {
    ...
    const response = await deletePaymentMethod(numericId)
    if (response.data?.code === 200) {
